/**
 * Migration utility to add the price_source column to the bots table
 */
...
async function addColumnIfNotExists(sequelize, tableName, columnName, dataType) {
  try {
    ...
    if (!exists) {
      ...
      const query = `ALTER TABLE ${tableName} ADD COLUMN IF NOT EXISTS ${columnName} ${dataType} DEFAULT 'three_commas'`;
...
async function runMigration() {
  try {
    console.log('Starting price_source column migration...');
    
    // Setup migration tracking table
    await createMigrationsTableIfNeeded();
    
    // Migration: Add price_source column to bots table
    const migrationId = '006_add_price_source_to_bots';
    ...
    if (!migrationApplied) {
      ...
      if (success) {
        ...
        await recordMigration(migrationId, 'Add price_source column to bots table');
